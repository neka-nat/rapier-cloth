...
ライブ用の材質は曲げを柔らかくし、球の周りにしわができ、端が折れやすい設定にしている。`bend_compliance = 1e3`、`stretch_compliance = 0` とし、面密度・減衰・摩擦は従来の値を使用する。complianceは大きいほど柔らかい。この値は32×32の離散制約用の調整値で、実在する生地の測定値ではない。
...
| 風の強さ | 時間と布上の位置によって変化する外力を調整する。布全体の揺れに局所的なたわみを加える簡易モデル。0なら風の外力はゼロ |
