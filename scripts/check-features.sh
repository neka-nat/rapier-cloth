...
task_matcher=(grep -Fq)
if command -v rg >/dev/null 2>&1; then task_matcher=(rg -Fq); fi
for task_crate in rapier-cloth-core rapier-cloth; do
    for task_features in none both; do
        ...
        if ! "${task_matcher[@]}" 'select exactly one precision feature: f32 or f64' "$task_log"; then
            cat "$task_log"
            exit 1
        fi
    done
